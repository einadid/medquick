<?php
// FILE: templates/nav_public.php (Final Responsive & Layout-Fixed Version)
...
<div x-data="{ searchOpen: false }" @open-search.window="searchOpen = true">
        ...
        <div class="container mx-auto px-4 sm:px-6">
            <div class="flex justify-between items-center py-4">
                ...
                <nav class="flex-1 flex justify-center space-x-8">
                    ...
                    <a href="#" class="text-base font-medium text-gray-500 hover:text-gray-900 transition-colors">About Us</a>
    ...
    <!-- Minimal Top Bar for Mobile (sm screens and smaller) -->
    <div class="md:hidden bg-white shadow-sm sticky top-0 z-40 px-4 py-3">
        <div class="flex justify-between items-center">
            ...
            <button @click="searchOpen = true" class="text-gray-600 hover:text-teal-600 p-2">
    ...
    <div class="md:hidden fixed bottom-0 left-0 right-0 h-[70px] bg-white border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] z-40">
        <div class="grid grid-cols-4 h-full">
            <a href="index.php" class="flex flex-col items-center justify-center pt-2 hover:bg-gray-50 transition-colors <?= $current_page == 'index.php' ? 'text-teal-600' : 'text-gray-500' ?>"><i class="fas fa-home text-xl"></i><span class="text-xs mt-1">Home</span></a>
            <a href="catalog.php" class="flex flex-col items-center justify-center pt-2 hover:bg-gray-50 transition-colors <?= $current_page == 'catalog.php' ? 'text-teal-600' : 'text-gray-500' ?>"><i class="fas fa-th-large text-xl"></i><span class="text-xs mt-1">Catalog</span></a>
            <a href="cart.php" class="flex flex-col items-center justify-center pt-2 hover:bg-gray-50 transition-colors relative <?= $current_page == 'cart.php' ? 'text-teal-600' : 'text-gray-500' ?>"><i class="fas fa-shopping-cart text-xl"></i><span id="cart-count-mobile" class="absolute top-2 right-4 text-xs bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center border-2 border-white hidden">0</span><span class="text-xs mt-1">Cart</span></a>
            <a href="login.php" class="flex flex-col items-center justify-center pt-2 hover:bg-gray-50 transition-colors <?= ($current_page == 'login.php' || $current_page == 'signup.php') ? 'text-teal-600' : 'text-gray-500' ?>"><i class="fas fa-user-circle text-xl"></i><span class="text-xs mt-1">Account</span></a>
        </div>
    </div>
    ...
    <!-- Reusable Search Logic Script (needed if nav_authenticated is not on the same page) -->
    <script>
